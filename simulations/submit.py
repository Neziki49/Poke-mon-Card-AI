...
sys.path.append(
    r"C:\Users\ujiki\Desktop\github\pokemon-tcg-ai-battle\simulations"
    )
...
# model4.pth must be placed in the same directory as main.py.
...
model = MyModel(
    ...
    2,    # num_heads
    256,  # d_feedforward
    1,    # num_layers_encoder
    1     # num_layers_decoder
)

# main.py and model4.pth are in the same directory.
model.load_state_dict(
    torch.load("model4.pth", map_location=device)
)

model = model.to(device)
model.eval()

# The deck used by this AI.
# This is the same deck that was used for training.
my_deck = [
...
def agent(obs_dict: dict) -> list[int]:
    ...
    Receives an observation and returns the selected action.
    """

    obs = to_observation_class(obs_dict)

    # At the beginning of the game, Kaggle asks for the deck.
    if obs.select is None:
        return my_deck

    # Use MCTS + neural network to select the action.
